...
installimage_file="/root/images/Debian-bookworm-latest-amd64-base.tar.gz"
...
if [ "$OS" == "PBS" ] ; then
  ...
  if [ ! -f postinstall_file="/root/pve" ] ; then
    wget "https://raw.githubusercontent.com/Deathgrint/Hetzner-Server/main/pve" -c -O /root/pve
  fi
